...
from architectures.mae_utils import get_2d_sincos_pos_embed
...
class Attention(nn.Module):
    ...
    def forward(self, x, pad_mask=None):
        ...
        if pad_mask is not None:
            attn[..., :, 1:] += pad_mask.reshape(B, 1, 1, -1) * -1e8  # TODO: Make sure it is correct
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, mask, patch_indices):
        # embed patches
...
def mae_vit_large_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(img_size=(IMG_SIZE[1], IMG_SIZE[0]),
